...
const TeacherLogin = ({ setIsLoggedIn }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await fetch(`${BACKEND_URL}/api/accounts/login/teacher`, {
        method: 'POST',
    ...
    <div style={{
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '2rem' }}>
          ...
          <h2 style={{ margin: 0, color: '#2563eb', fontWeight: 800, fontSize: '2rem', letterSpacing: '1px', textAlign: 'center' }}>RUET Teacher Login</h2>
        </div>
        ...
        <div style={{ textAlign: 'center', margin: '1.5rem 0 0 0', color: '#64748b', fontWeight: 500 }}>
          ...
          <Link to="/teacher-signup" style={{ color: '#2563eb', textDecoration: 'underline', fontWeight: 600 }}>
            Sign up
          </Link>
        </div>
        <div style={{ textAlign: 'center', margin: '0.5rem 0 0 0', color: '#64748b', fontWeight: 500 }}>
          Want to login as Head?{' '}
          <Link to="/head-login" style={{ color: '#2563eb', textDecoration: 'underline', fontWeight: 600 }}>
            Login
          </Link>
        </div>
      </div>
