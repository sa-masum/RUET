...
const HeadHeader = () => {
  ...
  const handleLogout = async () => {
    try {
      await fetch(`${BACKEND_URL}/api/accounts/logout/head`, {
        method: 'POST',
        credentials: 'include'
      });
      document.cookie = 'headToken=; Max-Age=0; path=/;';
      navigate('/head-login');
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', marginLeft: '2rem' }}>
        ...
        <span style={{ fontWeight: 800, fontSize: '1.3rem', color: '#2563eb', letterSpacing: '1px' }}>RUET Head Portal</span>
      </div>
